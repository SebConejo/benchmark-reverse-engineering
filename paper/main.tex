...
% --- Hyperlinks ---
\usepackage[breaklinks=true,colorlinks=true,linkcolor=OliveGreen,citecolor=OliveGreen,urlcolor=OliveGreen]{hyperref}
...
\author{S\'{e}bastien Conejo\thanks{sebastien@mnfst.com}}
\date{June 2026}
...
20 of 21 tasks (Mann-Whitney $p > 0.05$; $n_{\text{Premium}} = 2$).  Within providers, paying 150$\times$ more
...
\item \textbf{Empirical findings.}  Economy-tier models show no
  significant quality gap versus Premium on 20 of 21 tasks.  Per-task routing
...
\item \textit{No Economy--Premium gap detected.}  Mann-Whitney $p > 0.05$
  on 20 of 21 tasks (the single significant result favors Premium on
  instruction following).
  Premium averages 4.791 versus Economy 4.698, a gap of 0.093 on the
  1--5 scale.
\item \textit{Flat provider gradients.}  Anthropic Haiku (\$0.80/M)
  scores 4.775 while Opus (\$15/M) scores 4.753.  Google Flash
  (\$0.15/M) matches Pro (\$1.25/M).  OpenAI's 150$\times$ price range
  yields $+0.13$ quality.
...
\item \textit{Five Economy models cover all tasks.}  At the 4.0/5
...
test academic knowledge rather than production tasks.  The Berkeley
Function Calling Leaderboard covers a single task type.  TaskBench adds
...
\midrule
Premium  & $\geq$ \$5.00         & 2  \\
Standard & \$0.50--\$4.99        & 18 \\
Economy  & \$0.05--\$0.49        & 23 \\
Micro    & $<$ \$0.05            & 3  \\
\bottomrule
\end{tabular}
\end{center}
...
The five most discriminative tasks are multistep reasoning (spread
3.68), intent detection (CLINC-150, spread
3.20, $\sigma = 0.624$), extractive QA (spread 2.84, $\sigma = 0.560$),
content moderation (spread 2.40), and test generation (spread 2.40).  The
least discriminative is structured output (spread 0.29).
...
Standard & 4.756 & [4.722, 4.785] & 378 \\
Economy  & 4.698 & [4.665, 4.731] & 483 \\
Micro    & 4.104 & [3.904, 4.303] & 63  \\
\bottomrule
\end{tabular}
\end{center}

The Premium-to-Economy gap is 0.093 points (2.3\% of the 1--5 scale).
Confidence intervals overlap heavily.  Mann-Whitney tests find no
significant difference on 20 of 21 individual tasks ($p > 0.05$).  The
single significant result is instruction following, where Premium
outperforms Economy ($p = 0.050$).  With 21 simultaneous
tests at $\alpha = 0.05$, one rejection is expected by chance.

Micro is the only tier with a large gap ($-0.687$ versus Premium).  This
is driven by the three Micro models, which include the two sub-3B models
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/per_task_routing_savings}
\caption{Per-task routing savings versus o3.  Each bar shows the cost
reduction from routing to the cheapest model scoring $\geq 4.0$ on that
task.  All 21 tasks achieve $>98.7$\% savings.  The x-axis is zoomed to
98--100\% to make inter-task differences visible.}
\label{fig:routing}
\end{figure}


\subsection{Within-provider price gradient}
...
\textbf{OpenAI.}  Nano at \$0.10/M scores 4.694.  GPT-5.5~Pro at
\$15/M scores 4.828.  A 150$\times$ price increase yields $+0.134$, the
...
absolute gain ($+0.13$) across its 150$\times$ price range.  Mistral is
...
We now consider practical model selection strategies given the flat
price-quality gradient.
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/generational_delta}
\caption{Quality delta and price change for four generational model
pairs.  In three of four cases, the newer model is cheaper or equal in
price at higher quality.  Pairs selected where both predecessor and
successor have complete 21/21 task coverage.}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
\item \textbf{Default to Economy tier.}  On 20 of 21 tasks, Economy
  models showed no significant quality difference versus Premium (the
  single exception favors Premium on instruction following).  The safest default is the
...
\item \textbf{Model diversity.}  Our sample includes three geographic
  ...
  effect (Kruskal-Wallis $H = 3.80$, $p = 0.15$), and the
  open-versus-closed gap (0.191, $p = 0.0016$) is driven by sub-3B
...
cost-quality curve is flat: on 20 of 21 tasks we detect no significant
difference between Economy and Premium tiers, and the single significant
result favors Premium ($n_{\text{Premium}} = 2$; more Premium
...
\paragraph{Disclosure.}  This work was conducted by S\'{e}bastien Conejo,
...
\section{Full Pareto Frontiers (19 Remaining Tasks)}
\label{app:pareto}

One single-panel log-scale Pareto figure per task, following the style of
Figures~\ref{fig:pareto-rag} and~\ref{fig:pareto-data}.  RAG~QA and
Data-to-Text appear in the main text.

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures/pareto_sentiment_sst2}
\hfill
\includegraphics[width=0.48\textwidth]{figures/pareto_intent_clinc150}
\\[4pt]
\includegraphics[width=0.48\textwidth]{figures/pareto_moderation_toxigen}
\hfill
\includegraphics[width=0.48\textwidth]{figures/pareto_multistep_reasoning}
\\[4pt]
\includegraphics[width=0.48\textwidth]{figures/pareto_reasoning_gsm8k}
\hfill
\includegraphics[width=0.48\textwidth]{figures/pareto_code_generation}
\caption{Pareto frontiers (1/4): Classification and reasoning tasks.}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures/pareto_code_review_v2}
\hfill
\includegraphics[width=0.48\textwidth]{figures/pareto_code_explanation}
\\[4pt]
\includegraphics[width=0.48\textwidth]{figures/pareto_test_generation_v2}
\hfill
\includegraphics[width=0.48\textwidth]{figures/pareto_function_calling}
\\[4pt]
\includegraphics[width=0.48\textwidth]{figures/pareto_sql_spider}
\hfill
\includegraphics[width=0.48\textwidth]{figures/pareto_translation_enfr}
\caption{Pareto frontiers (2/4): Code and structured tasks.}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures/pareto_instruction_following}
\hfill
\includegraphics[width=0.48\textwidth]{figures/pareto_structured_output}
\\[4pt]
\includegraphics[width=0.48\textwidth]{figures/pareto_extraction_hard_v2}
\hfill
\includegraphics[width=0.48\textwidth]{figures/pareto_json_transform_v2}
\\[4pt]
\includegraphics[width=0.48\textwidth]{figures/pareto_email_summary_v2}
\hfill
\includegraphics[width=0.48\textwidth]{figures/pareto_long_summarization}
\caption{Pareto frontiers (3/4): Structured output and summarization tasks.}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures/pareto_ner_extraction}
\caption{Pareto frontier (4/4): NER extraction.}
\end{figure}
...
Translation BLEU is inconclusive ($r = 0.29$, weakly significant) due
to the metric's single-reference limitation, not the judge's quality.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures/judge_v2_code}
\hfill
\includegraphics[width=0.48\textwidth]{figures/judge_v2_translation}
\caption{Judge V2 validation scatter plots.  \textbf{Left:} Judge score
versus HumanEval pass@1 ($r = 0.88$, $p < 0.001$, $n = 48$).
\textbf{Right:} Judge score versus corpus BLEU ($r = 0.29$,
$p = 0.045$, $n = 48$).  Both include incomplete models.}
...
Three geographic groups: Chinese ($n = 16$, mean 4.753), American
($n = 25$, mean 4.656), and European ($n = 5$, mean 4.627).
Kruskal-Wallis $H = 3.80$, $p = 0.15$.  No significant difference.  The
American group has the widest range (3.261 to 4.842), driven by sub-3B
Llama models at the low end.

\paragraph{License type.}
Open-weight ($n = 17$, mean 4.566) versus closed-source ($n = 29$, mean
4.757).  Mann-Whitney $p = 0.0016$, gap 0.191.  However, the open-weight
group includes all sub-3B models (Llama~1B, Llama~3B, Ministral~3B).
Excluding sub-3B models narrows the gap to approximately 0.058.  The
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/task_discriminativeness}
\caption{Score spread (max $-$ min) per task across all 46 complete
models.  Red: spread $\geq 2.0$ (highly discriminative).  Blue:
$1.0$--$2.0$.  Green: $< 1.0$ (saturated).  Multistep reasoning has the
highest spread (3.68), while structured output is nearly saturated
(0.29).}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{figures/verbosity_scatter}
\caption{Average output tokens per model versus quality delta from the
grand mean.  Pearson $r = 0.217$, $p = 0.133$ (not significant).
Verbosity does not predict the V1-to-V2 score change, indicating that
rubric ambiguity --- not output length --- drives the format bias.}
\label{fig:verbosity}
\end{figure}


\section{Origin and License Comparisons}
\label{app:origin-license}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures/origin_comparison}
\hfill
\includegraphics[width=0.48\textwidth]{figures/license_comparison}
\caption{\textbf{Left:} Quality by provider origin (bootstrap 95\% CIs).
No significant difference (Kruskal-Wallis $p = 0.15$).
\textbf{Right:} Quality by license type (bootstrap 95\% CIs).
Open-weight scores lower ($p = 0.0016$), driven by sub-3B models.}
\label{fig:diversity}
\end{figure}
