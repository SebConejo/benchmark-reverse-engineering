...
total).  Economy-tier models (\$0.05--\$0.50/M input tokens) show no
statistically significant quality gap versus Premium models (\$5+/M) on
any of the 21 tasks (Mann-Whitney $p > 0.05$, 21/21).  Within
providers, paying 750$\times$ more buys at most $+0.13$ quality points
on a 1--5 scale.  Per-task routing to the cheapest adequate model
...
\item \textbf{TaskBench}, the first benchmark measuring per-task
  cost-quality Pareto frontiers across 46 models, 21 production tasks,
  9 providers, and 4 price tiers (51,580 scored cases).
\item \textbf{Empirical findings.}  Economy-tier models show no
  significant quality gap versus Premium on any task.  Per-task routing
...
\item \textit{No Economy--Premium gap detected.}  Mann-Whitney $p > 0.05$
  on 21 of 21 tasks.  Premium averages 4.791 versus Economy 4.749, a gap
  of 0.042 on the 1--5 scale.
\item \textit{Flat provider gradients.}  Anthropic Haiku (\$0.80/M)
  ...
  yields $+0.13$ quality.
\item \textit{Per-task routing saves 99.7\%.}  Routing to the cheapest
  adequate model per task costs \$0.000009/query versus \$0.003332 for
  o3, while matching o3 quality on every task.
...
RouterArena \citep{lu2024routerarena} evaluates 14 routers on 8{,}400
...
\item \textbf{Classification} (4 tasks, exact-match scoring): sentiment
  analysis (SST-2), intent detection (CLINC-150, 150 classes), content
  moderation (ToxiGen), and multistep reasoning (ARC-Challenge).
...
Each case has a fixed prompt.  Models receive identical inputs with
temperature $= 0$ (or $1$ for models that reject zero temperature).  Two
tasks have fewer than 50 cases due to dataset size: code explanation (48)
and structured output (41).

\subsection{Model Selection}

We evaluate 46 models that completed all 21 tasks with at least 40 valid
cases each, drawn from 9 API providers.  We assign models to four price
...
\midrule
Premium  & $\geq$ \$5.00         & 2  \\
Standard & \$0.50--\$4.99        & 22 \\
Economy  & \$0.05--\$0.49        & 13 \\
Micro    & $<$ \$0.05            & 9  \\
\bottomrule
\end{tabular}
\end{center}
...
(\$15/M).  Providers include OpenAI (10 models), Qwen (7), Mistral (5),
Anthropic (4), Google (3), ByteDance (3), xAI (3), Meta/Llama (3), and
others.
...
Judge scores versus pass@1 execution rate using a multi-strategy test
harness.  V2: $r = 0.891$.  A naive harness that failed on indentation
differences yielded only $r = 0.49$; our multi-strategy approach
(four extraction strategies tried in sequence) resolved 91.4\% of
formatting failures.
...
re-scored at approximately \$54).  The 14 tasks without ground truth are
...
\begin{figure}[t]
\centering
% NOTE: Convert SVG to PDF for LaTeX compilation.
% inkscape --export-type=pdf ../results/analysis_v3_final/heatmap_v3.svg
\includegraphics[width=\textwidth]{../results/analysis_v3_final/heatmap_v3}
...
Standard & 4.787 & [4.765, 4.808] & 451 \\
Economy  & 4.749 & [4.715, 4.777] & 313 \\
Micro    & 4.519 & [4.435, 4.599] & 210 \\
\bottomrule
\end{tabular}
\end{center}

The Premium-to-Economy gap is 0.042 points (0.9\% of the 1--5 scale).
Confidence intervals overlap heavily.  Mann-Whitney tests find no
significant difference on any of the 21 individual tasks ($p > 0.05$,
21/21).

Micro is the only tier with a meaningful gap ($-0.27$ versus Premium).
The practical reality is a three-tier world: Standard, Economy, and Micro
perform similarly, with Micro lagging on classification tasks where
sub-3B models fail.
...
detect a small gap.  Even if a gap exists, it is bounded below 0.12
(the difference between the upper CI of Premium and the lower CI of
Economy).

\begin{figure}[t]
\centering
\includegraphics[width=0.6\textwidth]{../results/analysis_v3_final/tier_comparison_v3}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../results/figures/pareto_rag_qa}
\caption{Pareto frontier for extractive QA (high-discrimination task).
Left panel: low-cost region showing the dense cluster of Economy and
Micro models near the frontier.  Right panel: full price range showing
Premium models far from the frontier.  Black line marks the Pareto
frontier.  Point shapes indicate price tiers.}
\label{fig:pareto-rag}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../results/figures/pareto_data_to_text}
\caption{Pareto frontier for data-to-text (commodity task).  Nearly all
models cluster above 4.8/5 regardless of price, illustrating a task
where model selection is irrelevant.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{../results/analysis_v3_final/provider_gradient}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
\item \textbf{Default to Economy tier.}  On 21 of 21 tasks, Economy
  models showed no significant quality loss versus Premium.  The safest
  default is the cheapest model scoring at least 4.5 on the target task.
...
TaskBench provides the first systematic per-task cost-quality benchmark
for production LLM use.  Across 46 models and 21 tasks, we find that the
cost-quality curve is remarkably flat: Economy models are statistically
indistinguishable from Premium on every task tested.  The practical
implication is clear.  Practitioners should default to Economy models and
...
All code, data, and evaluation prompts are publicly available at
\url{https://github.com/SebConejo/benchmark-reverse-engineering}.
...
One dual-panel Pareto figure per task, following the style of
